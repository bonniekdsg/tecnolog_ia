...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Power BI Modal Logic
  const pbiModal = document.getElementById('pbi-modal');
  const pbiModalContent = document.getElementById('pbi-modal-content');
  const pbiModalBackdrop = document.getElementById('pbi-modal-backdrop');
  const openPbiModalBtns = document.querySelectorAll('.open-pbi-modal');
  const closePbiModalBtn = document.getElementById('close-pbi-modal');

  function openPbiModal() {
    pbiModal.classList.remove('hidden');
    pbiModal.classList.add('flex');
    // trigger reflow
    void pbiModal.offsetWidth;
    // Fade in
    pbiModalBackdrop.classList.remove('opacity-0');
    pbiModalBackdrop.classList.add('opacity-100');
    // Scale in & Fade in content
    pbiModalContent.classList.remove('opacity-0', 'scale-95');
    pbiModalContent.classList.add('opacity-100', 'scale-100');
    document.body.style.overflow = 'hidden'; // prevent scrolling
  }

  function closePbiModal() {
    // Fade out
    pbiModalBackdrop.classList.remove('opacity-100');
    pbiModalBackdrop.classList.add('opacity-0');
    // Scale out & Fade out content
    pbiModalContent.classList.remove('opacity-100', 'scale-100');
    pbiModalContent.classList.add('opacity-0', 'scale-95');
    document.body.style.overflow = ''; // restore scrolling

    // Wait for transition to finish before hiding
    setTimeout(() => {
      pbiModal.classList.add('hidden');
      pbiModal.classList.remove('flex');
    }, 300);
  }

  openPbiModalBtns.forEach(btn => btn.addEventListener('click', (e) => {
    e.preventDefault();
    openPbiModal();
  }));

  closePbiModalBtn.addEventListener('click', closePbiModal);

  // Close when clicking outside the modal content
  pbiModal.addEventListener('click', (e) => {
    if (e.target === pbiModal || e.target === pbiModalBackdrop) {
      closePbiModal();
    }
  });

  // Close on ESC key for PBI modal
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && !pbiModal.classList.contains('hidden')) {
      closePbiModal();
    }
  });
});